/* LUNA EN VILLA PELÓN — JUGABILIDAD V1.3
 * Mobile-first, web-first. No external assets required.
 * Structure: collision map -> places -> directional sprites -> animation -> NPC life -> journal.
 */
(function(){
  const TILE=32;
  const P={
  ...
  /* ---------- WORLD GEOMETRY ---------- */
  const roads=[
    {x:90,y:560,w:2920,h:170},{x:1320,y:160,w:150,h:1650},{x:430,y:930,w:1760,h:120},{x:1880,y:1410,w:1160,h:120},
  ...
  const buildings=[
    ...
    {id:'almacen',name:'Almacén del pueblo',x:600,y:780,w:180,h:240},
    {id:'galpon',name:'Galpón comunitario',x:1280,y:980,w:260,h:130},
    {id:'escuela',name:'Escuela',x:2250,y:900,w:360,h:180},
    {id:'puesto',name:'Puesto rural',x:2850,y:430,w:300,h:150},
  ...
  function drawPerson(n,isPlayer=false){
    ...
    // Direction changes silhouette without adding image assets.
    if(st.dir==='up'){
  ...
  function memoryLabel(id){return ({'rosa-pista':'La pista de Rosa','tomas-pista':'La pista de Tomás','primer-recuerdo':'El primer recuerdo','almacen-recuerdo':'La vida cotidiana','escuela-recuerdo':'Aprender también es territorio','puente-recuerdo':'El río conecta el paisaje'})[id]||id;}
  ...
  function toggleJournal(force){createJournal();journalOpen=typeof force==='boolean'?force:!journalOpen;document.getElementById('journalPanel').classList.toggle('hidden',!journalOpen);if(journalOpen){renderJournal('objetivos');stopLoop();keys=Object.create(null);}else startLoop();}
  ...
  function worldLayout(){
    r(0,0,world.w,world.h,P.grass);roads.forEach(road);r(river.x,river.y,river.w,river.h,P.water);for(let y=100;y<2300;y+=42)r(river.x,y,river.w,4,P.water2);
    r(bridge.x,bridge.y,bridge.w,bridge.h,P.wood);r(bridge.x+10,bridge.y+10,bridge.w-20,12,'#b58b61');for(let x=bridge.x+20;x<bridge.x+bridge.w-10;x+=28)r(x,bridge.y,8,bridge.h,'#634936');
  ...
  function draw(){
    ...
    ctx.setTransform(viewport.dpr,0,0,viewport.dpr,0,0);ctx.clearRect(0,0,viewport.w,viewport.h);ctx.save();ctx.translate(-camera.x,-camera.y);worldLayout();animals.forEach(drawAnimal);for(const n of npcs)drawPerson(n);drawPerson({x:player.x,y:player.y,hair:'#513a30'},true);drawMarker();
  ...
  const pause=document.getElementById('pauseBtn');if(pause)pause.addEventListener('click',()=>togglePause());
  document.addEventListener('keydown',e=>{if(e.key==='i'||e.key==='I'){if(screen==='world')toggleJournal();}});
  createJournal();updateJournal();
  const closeReward=document.getElementById('closeReward');if(closeReward)closeReward.onclick=()=>{show('world');updateJournal();};
